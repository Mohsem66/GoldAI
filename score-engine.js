...
// =====================
// AI REASON FORMATTER v2
// =====================

function formatAIReasonV2(data){
    ...
    if(data.marketStructure){
        ...
        if(data.marketStructure.trend === "BULLISH"){
        ...
        else if(data.marketStructure.trend === "BEARISH"){
        ...
        if(data.marketStructure.structure){

            output.push(
                "Structure: "
                +
                data.marketStructure.structure
                +
                " تایید شد ✅"
            );
    ...
    if(data.ema){
        ...
        if(data.ema.buyScore > data.ema.sellScore){
        ...
        else if(data.ema.sellScore > data.ema.buyScore){
    ...
    if(data.rsi){
        ...
        if(
            ...
            output.push(
                "RSI: Healthy Momentum ⚡"
            );

        }


        else if(data.rsi.rsiValue >=70){

            output.push(
                "RSI: Overbought Warning ⚠️"
            );

        }


        else if(data.rsi.rsiValue <=30){

            output.push(
                "RSI: Oversold Reversal Zone ⚠️"
            );
