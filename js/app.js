...
window.GoldAI = {
  ...
  async fetchPriceFromBackend() {
    if (this.lastPrice && !this.isMarketOpen()) {
      window.GoldAI_Data.goldPrice = this.lastPrice;
    ...
    try {
      ...
      if (!window.GoldAI_Data.goldPrice) {
        const data = window.GoldAI_Data;
  ...
  // =============================================
  // تنظیمات پیشرفته یکپارچه (همه در یک مودال)
  // =============================================
  openAdvancedSettings() {
    const cfg = window.GoldAI_Config;
    const capital = window.GoldAI_Data.getCapital();
    const html = `
      <div id="advancedSettingsModal" class="modal">
        <div class="modal-content">
          ...
          <div class="settings-group">
            <label>سرمایه ($)</label>
            <input type="number" id="advCapital" value="${capital}" min="100" />
          </div>
          <div class="settings-group">
            <label>درصد ریسک (%)</label>
            <input type="number" id="advRisk" value="${cfg.DEFAULT_RISK_PERCENT}" min="0.1" max="10" step="0.1" />
          </div>
          <div class="settings-group">
            <label>استراتژی</label>
            <select id="advStrategy">
              <option value="scalp" ${cfg.STRATEGY_MODE === 'scalp' ? 'selected' : ''}>اسکالپ</option>
              <option value="swing" ${cfg.STRATEGY_MODE === 'swing' ? 'selected' : ''}>سوئینگ</option>
            </select>
          </div>
          <div class="settings-group">
            <label>تعداد TP</label>
            <select id="advTpCount">
              <option value="1" ${cfg.TP_COUNT === 1 ? 'selected' : ''}>۱</option>
              <option value="2" ${cfg.TP_COUNT === 2 ? 'selected' : ''}>۲</option>
              <option value="3" ${cfg.TP_COUNT === 3 ? 'selected' : ''}>۳</option>
            </select>
          </div>
          <div class="settings-group">
            <label>ضریب SL (ATR)</label>
            <input type="number" id="advSlMult" value="${cfg.ATR_SL_MULT}" step="0.1" min="0.5" max="5" />
          </div>
          <div class="settings-group">
            <label>ضریب TP1</label>
            <input type="number" id="advTp1Mult" value="${cfg.ATR_TP1_MULT}" step="0.1" min="0.5" max="10" />
          </div>
          <div class="settings-group">
            <label>ضریب TP2</label>
            <input type="number" id="advTp2Mult" value="${cfg.ATR_TP2_MULT}" step="0.1" min="0.5" max="15" />
          </div>
          <div class="settings-group">
            <label>ضریب TP3</label>
            <input type="number" id="advTp3Mult" value="${cfg.ATR_TP3_MULT}" step="0.1" min="0.5" max="20" />
          </div>
          <div class="settings-group">
            <label>لات دستی (۰=خودکار)</label>
            <input type="number" id="advUserLot" value="${cfg.USER_LOT || 0}" step="0.01" min="0" max="10" />
          </div>
          <div class="settings-actions">
            <button class="btn-main" onclick="GoldAI.saveAdvancedSettings()">✅ ذخیره همه تنظیمات</button>
            <button class="btn-ghost" onclick="GoldAI.closeAdvancedSettings()">❌ بستن</button>
          </div>
        </div>
      </div>
    `;
    document.getElementById('advancedSettingsModal')?.remove();
  ...
  saveAdvancedSettings() {
    ...
    const tpCount = parseInt(document.getElementById('advTpCount').value) || 3;
    const slMult = parseFloat(document.getElementById('advSlMult').value) || 1.5;
    const tp1Mult = parseFloat(document.getElementById('advTp1Mult').value) || 2.0;
    const tp2Mult = parseFloat(document.getElementById('advTp2Mult').value) || 3.5;
    const tp3Mult = parseFloat(document.getElementById('advTp3Mult').value) || 5.0;
    const userLot = parseFloat(document.getElementById('advUserLot').value) || 0;

    if (capital > 0) window.GoldAI_Data.setCapital(capital);
    window.GoldAI_Config.DEFAULT_RISK_PERCENT = risk;
    window.GoldAI_Config.STRATEGY_MODE = strategy;
  ...
  async runBacktest() {
    ...
    try {
      ...
      let wins = 0, losses = 0, total = 0, profit = 0, loss = 0, marketClosedCount = 0;
      for (let i = 50; i < closes.length - 5; i += 5) {
        const isOpen = this.isMarketOpen();
        if (!isOpen) { marketClosedCount++; continue; }
  ...
  getDailyWinRate() {
    ...
    let totalProfit = 0, totalLoss = 0, buys = 0, sells = 0, wins = 0, total = 0;
  ...
  render(r) {
    ...
    // انتقال توضیحات به پنل جزئیات
    const explanationEl = document.getElementById('signalExplanation');
    if (explanationEl) {
      let fullText = r.reason || '—';
      const L = r.layers || {};
      const AI = L.aiBrain || {};
      if (AI.reasoning) {
        fullText += '\n\n🧠 ' + AI.reasoning;
      ...
      if (!this.isMarketOpen()) {
        fullText += '\n\n⛔ بازار بسته است - این سیگنال فقط جنبه آموزشی دارد';
      }
      explanationEl.textContent = fullText;
    }
  ...
  showPanel(name) {
    ...
    if (name === "details" && this.lastResult) {
      this.render(this.lastResult);
      const explanationEl = document.getElementById('signalExplanation');
      if (explanationEl && this.lastResult) {
        let fullText = this.lastResult.reason || '—';
        const L = this.lastResult.layers || {};
        const AI = L.aiBrain || {};
        if (AI.reasoning) {
          fullText += '\n\n🧠 ' + AI.reasoning;
        }
