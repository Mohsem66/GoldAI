...
window.GoldAI = {
  ...
  // ===== دریافت قیمت از بک‌اند (با کش) =====
  async fetchPriceFromBackend() {
    // اگر قبلاً قیمت گرفته شده و بازار بسته است، از همان قیمت استفاده کن
  ...
  toggleSettingsPanel() {
    ...
    if (panel) {
      ...
      if (!panel.classList.contains('hidden')) {
        document.getElementById('advCapital').value = window.GoldAI_Data.getCapital();
        document.getElementById('advRisk').value = window.GoldAI_Config.DEFAULT_RISK_PERCENT;
        document.getElementById('strategySelect').value = window.GoldAI_Config.STRATEGY_MODE;
      }
    }
  },
  ...
  render(r) {
    ...
    // ===== انتقال توضیحات کامل به پنل جزئیات =====
    ...
    if (explanationEl) {
      ...
      // اضافه کردن اطلاعات اندیکاتورها به صورت خلاصه
      const details = [
        `EMA20: ${L.ema?.ema20 ? L.ema.ema20.toFixed(decs) : '—'}`,
        `EMA50: ${L.ema?.ema50 ? L.ema.ema50.toFixed(decs) : '—'}`,
        `RSI: ${L.rsi?.rsi ?? '—'}`,
        `ADX: ${L.adx?.adx ?? '—'}`,
        `ATR: ${L.atr?.atr ?? '—'}`,
        `MACD: ${L.macd?.hist ?? '—'}`,
        `Structure: ${L.structure?.structure ?? '—'}`,
        `Divergence: ${L.divergence?.type ?? '—'}`,
        `Liquidity: ${L.liquidity?.sweep ?? '—'}`,
        `Candle: ${L.candles?.pattern ?? '—'}`
      ];
      fullText += '\n\n📊 ' + details.join(' | ');
  ...
  showPanel(name) {
    ...
    if (name === "details" && this.lastResult) {
      ...
      // همچنین توضیحات را دوباره به‌روز می‌کنیم
      const explanationEl = document.getElementById('signalExplanation');
